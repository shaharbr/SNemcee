...
async def snec(Mzams, Ni_mass, E_final, Ni_boundary, R_CSM, K_CSM, semaphore):
    ...
    async with semaphore:
        ...
        dst_modelname_path = os.path.join(outputs_dir, name)
        if os.path.exists(dir_name) or os.path.exists(dst_modelname_path):
            progress_txt('skipped ' + name + ', already exists')
        elif E_final < 0.5 and Mzams > 12.0:
            progress_txt('skipped impossible SN ' + name)
        # if E_final > 0.5 and Ni_mass < 0.01:
        #     progress_txt('skipped unnecessary SN ' + name)
            ...
            dst_dat_path = os.path.join(outputs_dir, name, name + '_dat' + '.tar.gz')
            dst_xg_path = os.path.join(outputs_dir, name, name + '_xg' + '.tar.gz')
...
async def main():
    semaphore = asyncio.BoundedSemaphore(14)

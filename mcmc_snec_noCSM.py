...
import snec_model_interpolator_noCSM as interp
import pandas as pd
import os
import corner
import csv
            ...
            for Ni in ranges_dict['Ni']:
                ...
                for E in ranges_dict['E']:
                    ...
                    for Mix in ranges_dict['Mix']:
                        models[data_type][Mzams][Ni][E][Mix] = None
...
def load_model(Mzams, Ni, E, Mix, data_type):
    name = 'M' + str(Mzams) + \
           '_Ni' + str(Ni) + \
           '_E' + str(E) + \
           '_Mix' + str(Mix) + \
...
def load_surrounding_models(requested, ranges_dict, fitting_type, Tthreshold=False):
            ...
            for E in surrouding_values['E']:
                for Mix in surrouding_values['Mix']:
                    if 'lum' in fitting_type:
                        if models['lum'][Mzams][Ni][E][Mix] is None:
                            models['lum'][Mzams][Ni][E][Mix] = load_model(Mzams, Ni, E, Mix, 'lum')
                    if 'veloc' in fitting_type:
                        if models['veloc'][Mzams][Ni][E][Mix] is None:
                            models['veloc'][Mzams][Ni][E][Mix] = load_model(Mzams, Ni, E, Mix, 'veloc')
                    if 'mag' in fitting_type:
                        if models['mag'][Mzams][Ni][E][Mix] is None:
                            models['mag'][Mzams][Ni][E][Mix] = load_model(Mzams, Ni, E, Mix, 'mag')
                    if Tthreshold:
                        if models['temp'][Mzams][Ni][E][Mix] is None:
                            models['temp'][Mzams][Ni][E][Mix] = load_model(Mzams, Ni, E, Mix, 'temp')
...
def log_prior(theta, ranges_dict, nonuniform_priors=None):
    ...
    Parameters
    ----------
    theta: vector containing a specific set of parameters theta = [Mzams, Ni, E, Mix, S, T].
    ...
    if not theta_in_range(theta, ranges_dict):
        ...
        if nonuniform_priors is None:
            ...
            param_dict = {'Mzams': theta[0], 'Ni': theta[1], 'E': theta[2],
                          'Mix': theta[3], 'S': theta[4], 'T': theta[5]}
            log_prior = 0.0
...
def temp_thresh_cutoff(requested_theta, surrounding_values, models, data_x):
    temp_fit = interp.snec_interpolator(requested_theta[0:4], surrounding_values, models['temp'],
                                        data_x)
    max_x = 0
...
def calc_lum_likelihood(theta, data, surrounding_values, Tthreshold=False):
    data_x = data['t_from_discovery']
    data_x_moved = data_x - theta[5]
    if Tthreshold:
        max_x = temp_thresh_cutoff(theta[0:4], surrounding_values, models, data_x_moved)
        data_x_moved = data_x_moved[data_x_moved <= max_x]
        data = data.loc[data['t_from_discovery'] - theta[5] <= max_x]
    data_y = data['Lum']
    data_dy = data['dLum0']
    y_fit = interp.snec_interpolator(theta[0:4], surrounding_values, models['lum'], data_x_moved)
...
def calc_veloc_likelihood(theta, data, surrounding_values, Tthreshold=False):
    ...
    data_y = data['veloc']
    data_dy = data['dveloc']
    y_fit = interp.snec_interpolator(theta[0:4], surrounding_values, models['veloc'], data_x_moved)
    if not isinstance(y_fit, str):
        # calculate the log likelihood
        df = len(data_y) - 1
        return chi2.logpdf(chi_square_norm(data_y, data_dy, y_fit), df)
...
def calc_mag_likelihood(theta, data, surrounding_values, Tthreshold=False):
    log_likeli = 0
    ...
    for filt in filters:
        ...
        data_x_filt_moved = data_filt['t_from_discovery'] - theta[5]
        data_y_filt = data_filt['abs_mag']
        data_dy_filt = data_filt['dmag']
        y_fit[filt] = interp.snec_interpolator(theta[0:4], surrounding_values, models['mag'], data_x_filt_moved,
                                               filter=filt)
        if not isinstance(y_fit[filt], str):
            # multiply whole graph by scaling factor
            y_fit[filt] = y_fit[filt] * theta[4]
            # calculate the log likelihood
            df = len(data_x_filt_moved) - 1
            log_likeli += chi2.logpdf(chi_square_norm(data_y_filt, data_dy_filt, y_fit[filt]), df)
...
def log_likelihood(theta, data, ranges_dict, fitting_type):
    ...
    if theta_in_range(theta, ranges_dict):
        print('ok SN')
        surrounding_values = get_surrouding_values(theta[0:4], ranges_dict)
        load_surrounding_models(theta[0:4], ranges_dict, fitting_type, Tthreshold=True)
        if fitting_type == 'lum':
            log_likeli = calc_lum_likelihood(theta, data['lum'], surrounding_values)
        elif fitting_type == 'mag':
            log_likeli = calc_mag_likelihood(theta, data['mag'], surrounding_values, Tthreshold=True)
        elif fitting_type == 'veloc':
            log_likeli = calc_veloc_likelihood(theta, data['veloc'], surrounding_values, Tthreshold=True)
        elif fitting_type == 'lum-veloc':
            log_likeli = calc_lum_likelihood(theta, data['lum'], surrounding_values) + \
                         calc_veloc_likelihood(theta, data['veloc'], surrounding_values, Tthreshold=True)
        elif fitting_type == 'lum-veloc-normalized':
            num_obs_lum = len(data['lum']['t_from_discovery'])
            num_obs_veloc = len(data['veloc']['t_from_discovery'])
            log_likeli = calc_lum_likelihood(theta, data['lum'], surrounding_values) / num_obs_lum + \
                         calc_veloc_likelihood(theta, data['veloc'], surrounding_values, Tthreshold=True) / num_obs_veloc
        elif fitting_type == 'mag-veloc':
            log_likeli = calc_mag_likelihood(theta, data['mag'], surrounding_values, Tthreshold=True) + \
                         calc_veloc_likelihood(theta, data['veloc'], surrounding_values, Tthreshold=True)
        elif fitting_type == 'mag-veloc-normalized':
            num_obs_mag = len(data['mag']['t_from_discovery'])
            num_obs_veloc = len(data['veloc']['t_from_discovery'])
            log_likeli = calc_mag_likelihood(theta, data['mag'], surrounding_values, Tthreshold=True) / num_obs_mag + \
                         calc_veloc_likelihood(theta, data['veloc'], surrounding_values, Tthreshold=True) / num_obs_veloc
        elif fitting_type == 'combined':
            log_likeli = calc_lum_likelihood(theta, data['lum'], surrounding_values) + \
                         calc_mag_likelihood(theta, data['mag'], surrounding_values, Tthreshold=True) + \
                         calc_veloc_likelihood(theta, data['veloc'], surrounding_values, Tthreshold=True)
        elif fitting_type == 'combined-normalized':
            num_obs_lum = len(data['lum']['t_from_discovery'])
            num_obs_mag = len(data['mag']['t_from_discovery'])
            num_obs_veloc = len(data['veloc']['t_from_discovery'])
            log_likeli = calc_lum_likelihood(theta, data['lum'], surrounding_values) / num_obs_lum+ \
                         calc_mag_likelihood(theta, data['mag'], surrounding_values, Tthreshold=True) / num_obs_mag+ \
                         calc_veloc_likelihood(theta, data['veloc'], surrounding_values, Tthreshold=True) / num_obs_veloc
        else:
...
def emcee_fit_params(data, n_walkers, n_steps, ranges_dict, fitting_type, nonuniform_priors=None, init_guesses=None):
    ...
    sampler = emcee.EnsembleSampler(n_walkers, n_params, log_posterior,
                                    args=[data, ranges_dict, fitting_type, nonuniform_priors])
...
def chain_plots(sampler_chain, ranges_dict, output_dir, burn_in, first_stage_steps=None):
    ...
    for i in range(len(keys)):
        key = keys[i]
        plt.figure()
        plt.plot(sampler_chain[:, :, i].T, color='k', alpha=0.05)
...
def write_params_file(parameter_ranges, S_range, SN_name, n_walkers, n_steps, n_params, burn_in, time_now, output_dir):
...
def plot_lum_with_fit(data, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=False):
    data_x = data['t_from_discovery']
    data_y = data['Lum']
    dy0 = data['dLum0']
    dy1 = data['dLum1']
    ...
    for i in range(n_walkers):
        [Mzams, Ni, E, Mix, S, T] = sampler_chain[i, step, :]
        requested = [Mzams, Ni, E, Mix, S, T]
        if theta_in_range(requested, ranges_dict):
            load_surrounding_models(requested[0:4], ranges_dict, 'lum', Tthreshold=True)
            surrounding_values = get_surrouding_values(requested[0:4], ranges_dict)
            ...
            y_fit = interp.snec_interpolator(requested[0:4], surrounding_values, models['lum'], x_plotting)
            if not isinstance(y_fit, str):
                # multiply whole graph by scaling factor
                y_fit = y_fit * S
                ax.plot(x_plotting, np.log10(y_fit), alpha=0.4)
                log_likeli.append(calc_lum_likelihood(requested, data, surrounding_values, Tthreshold))
...
def plot_mag_with_fit(data, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=False):
    filters = list(data['filter'].unique())
    data_x = data['t_from_discovery']
    y_fit = {}
    f_fit, ax = plt.subplots(figsize=(10, 8))
    ...
    for i in range(n_walkers):
        ...
        requested = [Mzams, Ni, E, Mix, S, T]
        if theta_in_range(requested, ranges_dict):
            load_surrounding_models(requested[0:4], ranges_dict, 'mag', Tthreshold=True)
            surrounding_values = get_surrouding_values(requested[0:4], ranges_dict)
            data_x_moved = data_x - T
            if Tthreshold:
                max_x = temp_thresh_cutoff(requested, surrounding_values, models, data_x_moved)
                ...
                x_plotting = np.linspace(-T, 200-T, 2001)
            for filt in filters:
                y_fit[filt] = interp.snec_interpolator(requested[0:4], surrounding_values,
                                                       models['mag'], x_plotting, filter=filt)
                if not isinstance(y_fit, str):
                    # multiply whole graph by scaling factor
                    y_fit[filt] = y_fit[filt] * S
                    ax.plot(x_plotting, y_fit[filt], color=colors[filt], alpha=0.3)
                    log_likeli.append(calc_mag_likelihood(requested, data, surrounding_values, Tthreshold))
    log_likeli = np.mean(log_likeli)
    for filt in filters:
        data_filt = data.loc[data['filter'] == filt]
...
def plot_veloc_with_fit(data, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=False):
    data_x = data['t_from_discovery']
    data_y = data['veloc']
    data_dy = data['dveloc']
    f_fit, ax = plt.subplots(figsize=(10, 8))
    ...
    for i in range(n_walkers):
        ...
        requested = [Mzams, Ni, E, Mix, S, T]
        if theta_in_range(requested, ranges_dict):
            load_surrounding_models(requested[0:4], ranges_dict, 'veloc', Tthreshold=True)
            surrounding_values = get_surrouding_values(requested[0:4], ranges_dict)
            data_x_moved = data_x - T
            if Tthreshold:
                max_x = temp_thresh_cutoff(requested, surrounding_values, models, data_x_moved)
            ...
            y_fit = interp.snec_interpolator(requested[0:4], surrounding_values, models['veloc'], x_plotting_moved)
            if not isinstance(y_fit, str):
                ax.plot(x_plotting, y_fit, alpha=0.4)
                log_likeli.append(calc_veloc_likelihood(requested, data, surrounding_values, Tthreshold))
    log_likeli = np.mean(log_likeli)
...
def plot_lightcurve_with_fit(sampler_chain, SN_data, ranges_dict, fitting_type, output_dir, n_walkers, SN_name, step):
    ...
    if fitting_type == 'mag':
        log_likeli = plot_mag_with_fit(SN_data['mag'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True)
    ...
    if fitting_type == 'lum-veloc':
        ...
        log_likeli += plot_lum_with_fit(SN_data['lum'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name)
        log_likeli += plot_veloc_with_fit(SN_data['veloc'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True)
    if fitting_type == 'lum-veloc-normalized':
        log_likeli = 0
        ...
        log_likeli += plot_veloc_with_fit(SN_data['veloc'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True) / num_obs_veloc
    if fitting_type == 'mag-veloc':
        log_likeli = 0
        log_likeli += plot_mag_with_fit(SN_data['mag'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True)
        log_likeli += plot_veloc_with_fit(SN_data['veloc'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True)
    if fitting_type == 'mag-veloc-normalized':
        log_likeli = 0
        num_obs_veloc = len(SN_data['veloc']['t_from_discovery'])
        num_obs_mag = len(SN_data['mag']['t_from_discovery'])
        log_likeli += plot_mag_with_fit(SN_data['mag'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True) / num_obs_mag
        log_likeli += plot_veloc_with_fit(SN_data['veloc'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True) / num_obs_veloc
    if fitting_type == 'combined':
        ...
        log_likeli += plot_mag_with_fit(SN_data['mag'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True)
        log_likeli += plot_veloc_with_fit(SN_data['veloc'], sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=True)
    if fitting_type == 'combined-normalized':
        log_likeli = 0
        num_obs_mag = len(SN_data['mag']['t_from_discovery'])
        num_obs_veloc = len(SN_data['veloc']['t_from_discovery'])
        num_obs_lum = len(SN_data['lum']['t_from_discovery'])
...
def import_lum(SN_name):
    lum_path = os.path.join('results', SN_name + '_martinez.csv')
    data_lum = pd.read_csv(lum_path)
    return data_lum


def import_veloc(SN_name):
    veloc_path = os.path.join('results', SN_name + '_expansion_velocities.csv')
...
def import_mag(SN_name):
    mag_path = os.path.join('results', SN_name + '_lightcurves')
